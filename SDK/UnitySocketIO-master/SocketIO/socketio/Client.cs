...
namespace SocketIOClient
{
  ...
  public class Client : IDisposable, SocketIOClient.IClient
  {
    ...
    public EventHandler<MessageEventArgs> Message { get; set; }
    public EventHandler ConnectionRetryAttempt { get; set; }
    public EventHandler HeartBeatTimerEvent { get; set; }
    /// <summary>
    ...
    /// </summary>
    public EventHandler SocketConnectionClosed { get; set; }
    ...
    {
      OnErrorEvent(this, new ErrorEventArgs("wsClient_OpenEvent: init timer, interval:" + HandShake.HeartbeatInterval + " timeout:" + HandShake.HeartbeatTimeout + " conntimeout:" + HandShake.ConnectionTimeout));
      this.socketHeartBeatTimer = new Timer(OnHeartBeatTimerCallback, new object(), HandShake.HeartbeatTimeout * 1000, HandShake.HeartbeatTimeout * 1000);
      this.ConnectionOpenEvent.Set();

      this.OnMessageEvent(new EventMessage() { Event = "open" });
      if (this.Opened != null) {
        try { this.Opened(this, EventArgs.Empty); } catch (Exception ex) { OnErrorEvent(this, new ErrorEventArgs("wsClient_OpenEvent error", ex)); }
      }
    ...
    protected void OnSocketConnectionClosedEvent(object sender, EventArgs e)
    {
      if (this.SocketConnectionClosed != null) {
        try { this.SocketConnectionClosed(sender, e); } catch (Exception ex) {
          OnErrorEvent(this, new ErrorEventArgs("OnSocketConnectionClosedEvent error", ex));
        }
      }
    }
    protected void OnConnectionRetryAttemptEvent(object sender, EventArgs e)
    {
      if (this.ConnectionRetryAttempt != null) {
        try { this.ConnectionRetryAttempt(sender, e); } catch (Exception ex) {
          OnErrorEvent(this, new ErrorEventArgs("OnConnectionRetryAttemptEvent error", ex));
        }
      }
    }
    ...
    protected void OnHeartBeatTimerCallback(object state)
    {
      if (this.ReadyState == WebSocketState.Open) {
        ...
        try {
          ...
          OnErrorEvent(this, new ErrorEventArgs("OnHeartBeatTimerCallback error", ex));
        }
      }
    }
    private void EndAsyncEvent(IAsyncResult result)
    {
      ...
      try {
        ...
        OnErrorEvent(this, new ErrorEventArgs("EndAsyncEvent error", ex));
      }
    }
    /// <summary>
      ...
      while (this.outboundQueue != null) {
        if (this.ReadyState == WebSocketState.Open) {
          ...
          try {
            ...
            OnErrorEvent(this, new ErrorEventArgs("dequeuOutboundMessages error", ex)); 
          }
        } else {
    ...
    protected SocketIOHandshake requestHandshake(Uri uri)
    {
      ...
      using (WebClient client = new WebClient()) {
        try {
          ...
          this.OnErrorEvent(this, new ErrorEventArgs(errorText, ex));
        }
      }
      if (string.IsNullOrEmpty(errorText)) {
        this.OnErrorEvent(this, new ErrorEventArgs(value));
        handshake = SocketIOHandshake.LoadFromString(value); 
        this.OnErrorEvent(this, new ErrorEventArgs("handshake result:"+handshake.SID+","+handshake.HeartbeatTimeout+","+handshake.HeartbeatInterval));
      } else {
